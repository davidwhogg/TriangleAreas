...
\markboth{foo}{\scshape hainje \& hogg / A useless formula for the area of a triangle}
...
Although such a formula for the area of a triangle is of minimal, or even zero, practical interest, our search is motivated by real problems in physics.
...
Using this fact, we have written code to perform a symbolic regression which recovers our result.\footnote{
...
% \Hogg{Maybe drop this and replace it with a negative conclusion about learning nothing.}
% In this \documentname,
% we presented a new Deep Sets formula for the area of a triangle \eqref{eq:result} given the coordinates of the triangle's vertices.
% We extended it to triangles in higher-dimensional spaces \eqref{eq:quadrature}
% and to compute the volume of a simplex \eqref{eq:simplex}.
% We also considered other applications of the Deep Sets form, including to Heron's formula \eqref{eq:HeronDeepSets}
% and the gravitational potential energy of a system of point masses \eqref{eq:EnergyDeepSets}.
...
The authors also thank Ben Blum-Smith (JHU), Jonathan Goodman (NYU), Wilson Gregory (JHU), Teresa Huang (Flatiron), Mike O'Neil (NYU), Alexander Novara (NYU), Ben Wandelt (JHU), Kaze Wong (JHU), and the Blanton--Hogg group meeting at NYU for valuable discussions.
